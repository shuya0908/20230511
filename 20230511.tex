...
\begin{proof}
    ...
    Sect. 7.5 の最後の Remark より，2-dim. BM の再帰性から，開球 $\clb$ を $\abs{\Phi'}\ge q (>0)$ を満たすように選ぶと $P_z$-a.s. で $\int_0^\infty\bm{1}_{\clb}(B_s)ds=\infty$ なので
    $$
    C_\infty
    = \int_0^\infty\abs{\Phi'(B_s)}^2ds
    \ge q^2\int_0^\infty\bm{1}_{\clb}(B_s)ds
    = \infty.
    $$
