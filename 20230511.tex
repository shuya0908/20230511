...
\begin{screen}
    $\because)$（直感的な説明）
    時刻 $t$ までの $\theta$ の過去の挙動は $\lvert B_t\rvert$ の現在の値に関する情報を与える（例えば $\theta_t$ が時刻 $t$ の直前に非常に高速に振動する場合，必然的に動径 $\lvert B_t\rvert$ は小さくなる（動径が大きいならば偏角への影響は少ないため））．
    したがって過程 $\theta$ の未来の挙動に関する情報をも与える，つまり過去の挙動が未来の挙動に影響を与えるため，Markov性が成り立たない．
\end{screen}
