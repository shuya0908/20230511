...
言い換えると，$z$ スタートのcomplex BMの正則関数 $\Phi$ による像は，時間変更された $\Phi(z)$ スタートのcomplex BMとなる（この性質を2-dim. BMの\textbf{共形不変性 (conformal invariance property)}という）．
\begin{itemize}
    \item 
    Theorem \ref{thm:718}を $\Phi:D(\subset \compl)\to\compl\ (z\in D)$ が定まり，かつ正則である場合にまで拡張することが可能（多くの応用に有用）．
    \item 
