...
%DIF DEL before.tex   Mon May 22 17:50:32 2023
%DIF ADD after.tex    Mon May 22 17:50:06 2023
...
\newcommand\erase{\bgroup\markoverwith{\textcolor{red}{\rule[.5ex]{2pt}{0.4pt}}}\ULon} %eraseコマンドを定義 %DIF PREAMBLE
\providecommand{\DIFadd}[1]{{\protect\color{red}{#1}}} %DIF PREAMBLE
\providecommand{\DIFdel}[1]{{\protect\erase{#1}}}               %DIF PREAMBLE
...
\begin{proof}
    ...
    \begin{screen}
        ...
        $$
        \Phi'
        = \DIFdelbegin \DIFdel{\frac{d\Phi}{dz}
        = }\DIFdelend \frac{\partial g}{\partial x} - i\frac{\partial g}{\partial y}
        \DIFdelbegin \DIFdel{= }\DIFdelend \DIFaddbegin \beq{\text{C-R}} \DIFaddend \frac{\partial h}{\partial y} + i\frac{\partial h}{\partial x}
        \implies \abs{\Phi'}^2
        = \abs{\nabla g}^2
        = \abs{\nabla h}^2
        $$
    ...
    \DIFdelbegin \DIFdel{Exercise 4.24}\DIFdelend \DIFaddbegin \DIFadd{Sect. 7.5 の最後の Remark }\DIFaddend より\DIFdelbegin \DIFdel{$P_z$-a.s.で
    }\begin{displaymath}
    \DIFdel{\{C_\infty<\infty\}
...
%DIFDELCMD <     %%%
\DIFdel{一方}\DIFdelend ，2-dim. BM の再帰性\DIFdelbegin \DIFdel{より}\DIFdelend \DIFaddbegin \DIFadd{から}\DIFaddend ，開球 $\clb$ \DIFdelbegin \DIFdel{に対し $\set{t\ge0}{B_t\in\clb}$: 非有界であるため，$\Phi(B_t)=M_t+iN_t$ は有限な極限値}\DIFdelend を \DIFdelbegin \DIFdel{も}\DIFdelend \DIFaddbegin \DIFadd{$\abs{\Phi'}\ge q (>0)$ を満}\DIFaddend た\DIFdelbegin \DIFdel{ない．
    }\DIFdelend \DIFaddbegin \DIFadd{すように選ぶと $P_z$-a.s. で
    }$$
    \DIFadd{C_\infty
    ...
    \DIFaddend ゆえに $P_z$-a.s.で $C_\infty=\infty.$ \\
...
\begin{screen}
    \begin{thm*}[extended conformal invariance property]~
        \begin{itemize}
            ...
            $T:=\inf\{t\ge0:B_t\notin D\}$
            \item 
            \DIFdelbegin \DIFdel{$C_t=\int_0^{t\wedge T}\abs{\Phi'(B_s)}^2ds$ for $\Forall t\ge0$
            }\DIFdelend \DIFaddbegin \DIFadd{$C_t=\int_0^t\abs{\Phi'(B_s)}^2ds$ for $\Forall t\in [0, T]$
            }\DIFaddend \item 
            $(B_0=)z\in D$
        \end{itemize}
        \DIFdelbegin \DIFdel{$\implies \Exists \Gamma=(\Gamma_t)_{t\ge0}$}\DIFdelend \DIFaddbegin \DIFadd{$\implies \Exists \Gamma=(\Gamma_t)_{t\in [0, T]}$}\DIFaddend : $P_z$ の下で $\Phi(z)$ からスタートするcomplex BM s.t. $P_z$-a.s. \DIFdelbegin \DIFdel{$\Forall t\ge0, \Phi(B_t)^T=\Gamma_{C_t}.$
    }\DIFdelend \DIFaddbegin \DIFadd{$\Forall t\in [0, T], \Phi(B_t)=\Gamma_{C_t}.$
    }\DIFaddend \end{thm*}
\end{screen}

\begin{proof}
    \DIFdelbegin \DIFdel{$H_{\cdot}$: $C_{\cdot}$ }\DIFdelend \DIFaddbegin \DIFadd{$C_{\cdot}: [0, T]\ni t\mapsto\int_0^t\abs{\Phi'(B_s)}^2ds\in[0, \UB{S}{:=C_T}]$: 全単射より逆関数が存在する．
    $(H_s, s\in [0, S])$: $(C_t, t\in [0, T])$ }\DIFaddend の逆関数
    $\implies M_H, N_H$: indep. real BMsより $\Phi(B_H)$: complex BM.

    \begin{screen}
        $\because)$
        \DIFdelbegin \DIFdel{$\Forall t\ge0$ }\DIFdelend \DIFaddbegin \DIFadd{$\Forall t\in [0, T]$ }\DIFaddend に対し $\gen{M, M}_t=\gen{N, N}_t=C_t, \gen{M, N}_t=0$ より
        \begin{gather}
            \gen{M_{H}, M_{H}}_t
            = \gen{N_{H}, N_{H}}_t
            = C_{H_t}
    ...
    したがって \DIFdelbegin \DIFdel{$(\Phi(B_{H_s\wedge T}))_{s\ge0}$}\DIFdelend \DIFaddbegin \DIFadd{$(\Phi(B_{H_s}))_{s\in [0, S]}$}\DIFaddend : $\Phi(z)$ スタートのstopped complex BM under $P_z$.
    $$
    \Gamma_s
    := \Phi(B\DIFdelbegin \DIFdel{_{H_s\wedge T}}\DIFdelend \DIFaddbegin \DIFadd{_{H_s}}\DIFaddend )
    $$
    と定めると，$P_z$-a.s. \DIFdelbegin \DIFdel{$\Forall t\ge0$ }\DIFdelend \DIFaddbegin \DIFadd{$\Forall t\in [0, T]$ }\DIFaddend に対し \DIFdelbegin \DIFdel{$\Gamma_{C_t}=\Phi(B_{t\wedge T})=\Phi(B_t)^T$}\DIFdelend \DIFaddbegin \DIFadd{$\Gamma_{C_t}=\Phi(B_t)$}\DIFaddend （\DIFdelbegin \DIFdel{$C_T$ }\DIFdelend \DIFaddbegin \DIFadd{$S$ }\DIFaddend は $\Gamma$ に\DIFdelbegin \DIFdel{おけ}\DIFdelend \DIFaddbegin \DIFadd{よ}\DIFaddend る $\Phi(D)$ からのfirst exit timeとなる）．
\end{proof}
...
\begin{proof}
    ...
    ここで \DIFdelbegin \DIFdel{$H_{\cdot}$ }\DIFdelend \DIFaddbegin \DIFadd{$(H_s, s\ge0)$ }\DIFaddend を \DIFdelbegin \DIFdel{$C_{\cdot}$ }\DIFdelend \DIFaddbegin \DIFadd{$(C_t, t\ge0)$ }\DIFaddend の逆関数とすると，逆関数定理 ($(f^{-1})'(f(a))=1/f'(a)$) より
    \begin{align}
        H_s
    ...
    $B_t$ の偏角の連続な determination \DIFdelbegin %DIFDELCMD < \nazo %%%
...
\DIFdelend \DIFaddbegin \begin{gather}
    ...
    \DIFaddbegin \DIFadd{以上より
    }\begin{itemize}
        \item 
        \DIFaddend $\beta, \gamma$\DIFdelbegin \DIFdel{は }\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend $B$ の決定論的な関数
        \DIFdelbegin \DIFdel{な}\DIFdelend \DIFaddbegin \item 
        \DIFadd{$\Gamma^1, \Gamma^2$: $Z$ }\DIFaddend の\DIFaddbegin \DIFadd{決定論的な関数
        }\item 
        \DIFadd{$(B_t)_{t\ge0}\beq{\text{(d)}}(Z_t)_{t\ge0}$
    }\end{itemize}

    \DIFadd{$\implies (\beta_t+i\gamma_t)_{t\ge0}\beq{\text{(d)}}(\Gamma_t)_{t\ge0}$ }\DIFaddend で\DIFdelbegin \DIFdel{$(B_t, t\ge0)\beq{\text{(d)}}(Z_t, t\ge0).$
    したがって $(\beta_t+i\gamma_t, t\ge0)\beq{\text{(d)}}(\Gamma_t, t\ge0)$ で}\DIFdelend あるため所望の結果を得る．
\end{proof}
...
これは \DIFdelbegin \DIFdel{$(\lvert B_t\rvert, t\ge0)$ }\DIFdelend \DIFaddbegin \DIFadd{$(\lvert B_t\rvert)_{t\ge0}$ }\DIFaddend がMarkov過程，i.e. 2-dim. Bessel過程であることと関係している（Bessel過程の簡単な議論はExercise 6.24やSect. 8.4.3を参照）．

一方，\DIFdelbegin \DIFdel{$\theta_t:=\arg B_t=\gamma_{H_t}\implies \theta=(\theta_t, t\ge0)$ }\DIFdelend \DIFaddbegin \DIFadd{$\theta_t:=\arg B_t=\gamma_{H_t}\implies \theta=(\theta_t)_{t\ge0}$ }\DIFaddend はMarkov過程でない．

\begin{screen}
    $\because)$（直感的な説明）
    時刻 $t$ までの $\theta$ の過去の挙動は $\lvert B_t\rvert$ の現在の値に関する情報を与える（例えば $\theta_t$ が時刻 $t$ の直前に非常に高速に振動\DIFdelbegin \DIFdel{した}\DIFdelend \DIFaddbegin \DIFadd{する}\DIFaddend 場合，\DIFaddbegin \DIFadd{必然的に動径 }\DIFaddend $\lvert B_t\rvert$ は小さくなる\DIFaddbegin \DIFadd{（動径が大きいならば偏角への影響は少ないため}\DIFaddend ）\DIFdelbegin %DIFDELCMD < \nazo%%%
\DIFdelend \DIFaddbegin \DIFadd{）}\DIFaddend ．
    したがって過程 $\theta$ の未来の挙動に関する情報をも\DIFaddbegin \DIFadd{与える，つまり過去の挙動が未来の挙動に影響を}\DIFaddend 与えるため，Markov性が成り立たない．
\end{screen}
